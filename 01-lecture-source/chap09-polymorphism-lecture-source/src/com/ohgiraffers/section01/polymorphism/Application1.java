...
//        Rabbit r = new Animal(); // 에러남, 더 큰 타입을 작은 타입에 넣으려해서
//        Tiger t = new Animal();
        ...
        * 런타입 당시의 실제 객체가 가진 오버라이딩된 메소드로 바인딩이 바뀌어 동작하는 것을 의미한다.
        ...
        a1.cry(); // 실제 a1에 담긴건 Rabbit이다
        a2.cry();

//        a1.jump(); // 에러남, 자료형 타입이 Animal이기 때문에 Rabbit, Tiger가 가진 고유 메소드는 동작하지 않는다.
//        a2.bite(); // 에러남
...
//        ((Tiger)a1).bite(); // Class case exception, 토끼는 호랑이가 될 수 없다.

        /* 8. instanceof 연산자 사용 확인 */ // (래퍼런스명 instanceof 클래스명)
        ...
        System.out.println("a1이 Object 타입인지 확인 : " + (a1 instanceof Object)); // 모든 클래스는 Object의 자식이다.
...
/*        Animal animal1 = new Rabbit(); // up-casting 묵시적 형변환
        ...
        Rabbit rabbit2 = animal2; // donw-casting 묵시적 형변환이 안됨*/
    }
}
